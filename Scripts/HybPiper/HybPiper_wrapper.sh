...
for i in $(eval echo {1..$Total}); do
	...
	reads_first.py -r ${forward} ${reverse} -b /ufrc/soltis/cactus/Dimension/Target_Enrichment/data/Angiosperms353_targetSequences.fasta --cpu 1 --bwa --prefix ${name} --unpaired ${Unpaired}
done
